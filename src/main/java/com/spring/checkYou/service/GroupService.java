...
public class GroupService {
	...
	// task 그룹 페이지로 이동 : DB접근해서 데이터를 그래프에 뿌려줌
	public void task_group(Model model) {

	}

	// 그룹 멤버의 작업 추가
	public String addWorkProgress(GroupTaskDto dto) {
		IGroupDao dao = sqlSession.getMapper(IGroupDao.class);

		String check = dao.checkGroupTask(dto);
		System.out.println("checkwork query test : " + check);
		String result = null;
		if (check == null) {
			System.out.println("success adding newGroupTask");
			dao.addWorkProgress(dto);
			result = "success";

		} else {
			System.out.println("this GroupTask is already exist");
			result = "exist";
		}
	...
	// 현재 작업 목록
	public List<GroupTaskDto> taskList() {
		...
		List<GroupTaskDto> taskList = sqlSession.selectList("taskList", dto);

		return taskList;
	}

	// 완료된 작업
	public List<GroupTaskDto> completedList() {
		GroupTaskDto dto = new GroupTaskDto();
		String groupname = (String) session.getAttribute("selectedGroup");
		String username = (String) session.getAttribute("userId");
		...
		List<GroupTaskDto> completedList = sqlSession.selectList("completedList", dto);

		return completedList;
	}

	// Mission 목록
	public List<GroupTaskDto> missionList() {
		GroupTaskDto dto = new GroupTaskDto();
		String groupname = (String) session.getAttribute("selectedGroup");
		String username = (String) session.getAttribute("userId");
		...
		List<GroupTaskDto> missionList = sqlSession.selectList("missionList", dto);
	...
	public void updateProgress(GroupTaskDto dto) {
		IGroupDao dao = sqlSession.getMapper(IGroupDao.class);
		dao.updateProgress(dto);
	}

	// 작업 삭제
	public void deleteProgress(GroupTaskDto dto) {
		IGroupDao dao = sqlSession.getMapper(IGroupDao.class);
		dao.deleteProgress(dto);
	}
